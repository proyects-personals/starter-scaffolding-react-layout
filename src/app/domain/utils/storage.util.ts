...
export class StorageUtil extends ParseValueUtil {
   ...
   * @returns Resultado de la operación con estado de éxito o error.
   */
   ...
   * @param fallback Valor por defecto y guía de tipo si la clave no existe o es inválida.
   * @returns Objeto con el valor tipado y metadatos de éxito.
   * @example
   ...
   * @param value Valor a persistir (strings se guardan directos, otros tipos vía JSON.stringify).
   * @returns Confirmación de la persistencia.
   */
   ...
   * @returns `true` si la clave existe, `false` en caso contrario o error.
   */
